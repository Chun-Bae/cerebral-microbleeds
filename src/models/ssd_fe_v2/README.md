# SSD_FE_V2 (Cerebral Microbleeds Model)

이 문서는 `SSD_FE_V2` 모델(Cerebral Microbleeds 탐지용 Object Detection 신경망)의 아키텍처 및 내부 구조에 대한 상세한 설명을 기술합니다.
`ssd_fe_v2` 폴더는 `ssd_fe_v1` 구조를 기반으로 하면서, **더 얕은 레이어(conv3 블록 내부)에서만 앵커를 뽑는 실험적 아키텍처**입니다. V1이 conv3_3 → conv4_3 → conv5_3 순으로 깊이 내려가며 다중 스케일을 구성했다면, V2는 **conv3 블록(conv3_1 / conv3_2 / conv3_3) 내에서만** 3개의 스테이지를 구성합니다.

## 1. 개요 (Overview)
* **Backbone:** VGG-16 (`torchvision.models.vgg16`)
* **해상도 지원 (Input Size):** 기본적으로 512×512 크기의 영상을 상정하여 Feature Map 사이즈를 잡음
* **특징 (Features):**
  * 3개의 스테이지가 모두 `conv3` 블록 내에 위치 → 모든 Feature Map이 **128×128, 256ch** 동일
  * Pooling 횟수를 최소화하여 공간 정보 손실을 줄임
  * `Feature Enhancement (FE)` 레이어를 `Stage 2(conv3_2)` 위에 씌워서 병변 의심 부위의 강도를 증폭시키는 기법 활용

---

## 2. 예측 헤드 및 Feature Map 스테이지 (Heads & Stages)

이 모델은 VGG-16 백본의 conv3 블록에서 총 **3개의 계층(Feature Maps)** 을 뽑아와 Head(`loc`, `conf`)를 거치게 됩니다.

| Stage | VGG Block | VGG16 index | Feature Map Size | In Channels | 출력 Head / Anchors (비율) |
| :---: | :---: | :---: | :---: | :---: | :--- |
| **Stage 1** | `conv3_1` | `[:12]` | $128 \times 128$ | 256 | `SSDHead(256)`, 4 anchors ($r \in \{0.875, 1, 1.125\}$) |
| **Stage 2** | `conv3_2` | `[12:14]` | $128 \times 128$ | 256 | `SSDHead(256)`, 4 anchors ($r \in \{0.875, 1, 1.125\}$) |
| **Stage 3** | `conv3_3` | `[14:16]` | $128 \times 128$ | 256 | `SSDHead(256)`, 4 anchors ($r \in \{0.875, 1, 1.125\}$) |
...
## 4. `SSD_FE_V1` 과의 가장 큰 차이

| 항목 | V1 | V2 |
| :---: | :--- | :--- |
| **Stage 구성** | conv3_3 → conv4_3 → conv5_3 | conv3_1 → conv3_2 → conv3_3 |
| **Feature Map 크기** | 128×128 / 64×64 / 32×32 | 128×128 / 128×128 / 128×128 |
| **In Channels** | 256 / 512 / 512 | 256 / 256 / 256 |
| **FE 적용 위치** | Stage 2 (conv4_3) | Stage 2 (conv3_2) |
| **Pooling 횟수** | 3회 (pool1~3 통과) | 2회 (pool1~2 통과) |

V2는 conv3 블록 전체를 단계별로 쪼개어, **풀링 없이 동일 해상도(128×128)** 에서 점진적으로 쌓이는 특징을 관찰하는 실험입니다. 다중 해상도 대신 **다중 깊이(depth)** 를 활용하는 구조이므로, V1 대비 공간 정보 손실은 더 적지만 의미론적(semantic) 정보는 더 얕습니다.
