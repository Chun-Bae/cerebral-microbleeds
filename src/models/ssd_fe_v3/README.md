# SSD_FE_V3 (Cerebral Microbleeds Model)

이 문서는 `SSD_FE_V3` 모델(Cerebral Microbleeds 탐지용 Object Detection 신경망)의 아키텍처 및 내부 구조에 대한 상세한 설명을 기술합니다.
`ssd_fe_v3` 폴더는 `ssd_fe_v2` 구조를 기반으로 하면서, **V2의 얕은 스테이지(conv3_2, conv3_3)를 유지하면서 conv4_3을 마지막 스테이지로 추가한 하이브리드 아키텍처**입니다. V2가 conv3 블록 내에서만 동작했다면, V3는 **conv3_2 → conv3_3 → conv4_3** 으로 한 단계 더 내려가 multi-scale을 부분적으로 회복합니다.

## 1. 개요 (Overview)
* **Backbone:** VGG-16 (`torchvision.models.vgg16`)
* **해상도 지원 (Input Size):** 기본적으로 512×512 크기의 영상을 상정하여 Feature Map 사이즈를 잡음
* **특징 (Features):**
  * Stage 1~2는 `conv3` 블록 내에서 **128×128, 256ch** 동일 해상도
  * Stage 3은 `conv4_3`으로 내려가 **64×64, 512ch** 의 다중 스케일 추가
  * `Feature Enhancement (FE)` 레이어를 `Stage 2(conv3_3)` 위에 씌워서 병변 의심 부위의 강도를 증폭시키는 기법 활용
...
| :---: | :---: | :---: | :---: | :---: | :--- |
| **Stage 1** | `conv3_2` | `[:14]` | $128 \times 128$ | 256 | `SSDHead(256)`, 4 anchors ($r \in \{0.875, 1, 1.125\}$) |
| **Stage 2** | `conv3_3` | `[14:16]` | $128 \times 128$ | 256 | `SSDHead(256)`, 4 anchors ($r \in \{0.875, 1, 1.125\}$) |
...
## 4. `SSD_FE_V1 / V2` 와의 비교

| 항목 | V1 | V2 | V3 |
| :---: | :--- | :--- | :--- |
| **Stage 구성** | conv3_3 → conv4_3 → conv5_3 | conv3_1 → conv3_2 → conv3_3 | conv3_2 → conv3_3 → conv4_3 |
| **Feature Map 크기** | 128×128 / 64×64 / 32×32 | 128×128 / 128×128 / 128×128 | 128×128 / 128×128 / 64×64 |
| **In Channels** | 256 / 512 / 512 | 256 / 256 / 256 | 256 / 256 / 512 |
| **FE 적용 위치** | Stage 2 (conv4_3) | Stage 2 (conv3_2) | Stage 2 (conv3_3) |
| **Pooling 횟수** | 3회 (pool1~3 통과) | 2회 (pool1~2 통과) | 3회 (pool1~3 통과) |

V3는 V2의 얕은 첫 두 스테이지(conv3_2, conv3_3)를 유지하면서 conv4_3을 마지막으로 추가한 **하이브리드 구조**입니다. V2 대비 FP 증가 없이 Recall을 일부 회복하는 것을 목표로 합니다. conv5_3을 제거해 V1의 깊은 semantic feature로 인한 FP를 억제하면서, conv4_3의 64×64 multi-scale로 V2가 놓친 탐지를 보완합니다.
